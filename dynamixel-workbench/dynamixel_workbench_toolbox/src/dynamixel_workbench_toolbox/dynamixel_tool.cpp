...
{  
  ...
  else if (!strncmp(name, "XC430-W250_2", strlen(name)))
    setControlTable(XL430_W250);
...
void DynamixelTool::setModelName(uint16_t model_number)
{
  ...
  else if (num == XC430_W250_2)
    strcpy(dxl_info_[dxl_info_cnt_].model_name, "XC430-W250_2");
...
void DynamixelTool::setModelNum(const char* model_name)
{
  ...
  if (!strncmp(name, "AX-12A", strlen(name)))
    ...
    dxl_info_[dxl_info_cnt_].model_num = XC430_W250_2;
